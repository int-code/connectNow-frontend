...
const Signup: React.FC<Props> = () => {
  ...
  useEffect(() => {
    const submitData = async () => {
      if (isSubmitting) {
        ...
        try {
          const response = await fetch('https://connectnow-backend-fwe9.onrender.com/auth/signup', {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    const verifyCode = async () => {
      if (isVerifying) {
        try {
          const response = await fetch('https://connectnow-backend-fwe9.onrender.com/auth/verify', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const handleCheck = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    try {
      const response = await fetch('https://connectnow-backend-fwe9.onrender.com/auth/checkUsername', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
