    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.status === 401) {
          ...
          if (refreshToken) {
            const refreshResponse = await fetch(`http://127.0.0.1:8000/auth/refresh/?refresh_token=${refreshToken}`, {
              method: "POST",
            });
            ...
            response = await fetch(`http://127.0.0.1:8000/user/${user_id}`, {
              headers: {
        ...
        // Fetch user projects
        response = await fetch(`http://127.0.0.1:8000/project/user/${user_id}`, {
          headers: {
        ...
        response = await fetch(`http://127.0.0.1:8000/skill/user/${user_id}`, {
          headers: {
            "access-token": accessToken || '',
          },
        });
